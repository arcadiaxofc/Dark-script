-- ============================================================
-- NEXUS v7.0.9 - COMPLETO PARA DELTA EXECUTOR
-- ============================================================
...
local function showNearbyEntities()
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("Highlight") and obj.Name == "NexusHighlight" then
            obj:Destroy()
        end
    end
    ...
    for _, obj in pairs(Workspace:GetDescendants()) do
        if obj:IsA("Model") and obj ~= player.Character and obj:FindFirstChild("Humanoid") and obj:FindFirstChild("HumanoidRootPart") then
            ...
            if dist <= range then
                local entityType = "Mob"
                local color = Color3.fromRGB(150, 150, 150)
                
                if isQuestNPC(obj) then 
                    entityType = "QUEST"
                    color = Color3.fromRGB(0, 255, 0)
                elseif obj:FindFirstChild("Humanoid") and obj.Humanoid.MaxHealth > 10000 then 
                    entityType = "BOSS"
    ...
    print("========== ENTIDADES PRÓXIMAS (" .. #entities .. ") ==========")
    for i, e in pairs(entities) do
        if i <= 15 then
            print(string.format("[%s] %s | ❤%d/%d | %dm", e.type, e.name, e.health, e.maxHealth, e.dist))
        end
    end
    if #entities > 15 then print("... e mais " .. (#entities - 15) .. " entidades") end
    print("==========================================")
...
function SuperFarm.createBox() 
    ...
    SuperFarm.Box.Size = Vector3.new(10, 5, 10) 
    SuperFarm.Box.Anchored = true 
    SuperFarm.Box.CanCollide = false 
    SuperFarm.Box.Transparency = 1 
end
...
function SuperFarm.update()
    ...
    if SuperFarm.Phase == "quest" and SuperFarm.AutoQuest and tick() - SuperFarm.LastQuest > 45 then
        local entities = showNearbyEntities()
        ...
        if nearest then
            print("✅ NPC DE QUEST ENCONTRADO: " .. nearest.Name)
            hrp.CFrame = CFrame.new(nearest.HumanoidRootPart.Position + Vector3.new(0, 25, 0)) 
            task.wait(0.1)
            hrp.CFrame = CFrame.new(nearest.HumanoidRootPart.Position + Vector3.new(3, 0, 0)) 
            task.wait(0.5)
            
            local r = ReplicatedStorage:FindFirstChild("Remotes") 
            if r and r:FindFirstChild("CommF_") then 
                r.CommF_:InvokeServer("StartQuest", nearest.Name) 
                print("📋 Quest iniciada: " .. nearest.Name) 
            end
            
            SuperFarm.LastQuest = tick() 
            SuperFarm.Killed = 0 
            SuperFarm.Phase = "collect"
        else
            print("❌ NENHUM NPC DE QUEST ENCONTRADO!")
        end
    end
        ...
        for _, obj in pairs(Workspace:GetDescendants()) do
            ...
            if isEnemy(obj) and not SuperFarm.Mobs[obj] then
                ...
                if mHrp and (mHrp.Position - hrp.Position).Magnitude <= 500 then
                    ...
                    c = c + 1
                    print("📦 Puxado: " .. obj.Name)
                end
            end
        end
        if c > 0 then 
            SuperFarm.Phase = "attack" 
            print("⚔️ Fase: ATAQUE (" .. c .. " mobs)") 
        end
    end
        ...
        for obj, _ in pairs(SuperFarm.Mobs) do
            if obj:FindFirstChild("Humanoid") and obj:FindFirstChild("HumanoidRootPart") then
                if obj.Humanoid.Health > 0 then
                    ...
                    if (mHrp.Position - SuperFarm.Box.Position).Magnitude > 10 then 
                        mHrp.CFrame = CFrame.new(SuperFarm.Box.Position + Vector3.new(math.random(-3, 3), 0, math.random(-3, 3))) 
                    end
                    ...
                    print("💀 Morto: " .. obj.Name .. " (" .. SuperFarm.Killed .. "/" .. SuperFarm.Needed .. ")") 
                end
            else 
                SuperFarm.Mobs[obj] = nil 
                SuperFarm.Killed = SuperFarm.Killed + 1 
    ...
    -- Auto Collect Fruits
    if SuperFarm.AutoCollect and tick() % 10 < 0.1 then 
        for _, obj in pairs(Workspace:GetDescendants()) do 
    ...
    -- Auto Buy
    if SuperFarm.AutoBuy and tick() % 300 < 0.1 then 
        local r = ReplicatedStorage:FindFirstChild("Remotes") 
        if r and r:FindFirstChild("CommF_") then 
...
function startKillAura() 
    startThread("killaura", function() 
        ...
        local hrp = player.Character.HumanoidRootPart 
        for _, o in pairs(Workspace:GetDescendants()) do 
            if isEnemy(o) and o:FindFirstChild("HumanoidRootPart") and (o.HumanoidRootPart.Position - hrp.Position).Magnitude < range then 
                hrp.CFrame = CFrame.new(hrp.Position, o.HumanoidRootPart.Position) 
                attack() 
            end 
        end 
    end, 0.1) 
...
RunService.RenderStepped:Connect(function()
    ...
    local nw = tick()
    if nw - lt >= 1 then
        fpsLabel.Text = "FPS: " .. fc .. " | 💀 Kills: " .. kills
        fc = 0
        lt = nw
    end
end)
...
local tabs = {}
for _, t in pairs({
    {"⚔️ Farm"}, {"🎯 Bosses"}, {"🍎 Frutas"}, {"💎 Farms"}, 
    {"🏃 Move"}, {"⚙️ Auto"}, {"🛍️ Loja"}, {"👀 ESP"}, 
    {"🎮 Extra"}, {"🏝️ Ilhas"}
}) do 
...
NexusUI:CreateToggle(tabs["⚔️ Farm"], {
    Title = "🚀 Super Farm", 
    Callback = function(v) 
        if v then startSuperFarm() else stopSuperFarm() end 
    end
})
NexusUI:CreateToggle(tabs["⚔️ Farm"], {
    Title = "💀 Kill Aura", 
...
NexusUI:CreateButton(tabs["⚔️ Farm"], {
    Title = "🔍 VER ENTIDADES PRÓXIMAS", 
    Callback = function() 
        showNearbyEntities() 
        notify("🔍", "Veja o console (F9)!", 5) 
    end
})
...
for _, name in pairs(BOSSES[currentSea]) do
    NexusUI:CreateToggle(tabs["🎯 Bosses"], {
        Title = "🎯 " .. name, 
        Callback = function(v) 
            if v then startBossFarm(name) else stopBossFarm(name) end 
        end
    })
end
...
NexusUI:CreateToggle(tabs["🍎 Frutas"], {
    ...
    Callback = function(v) 
        if v then startAutoStore() else stopAutoStore() end 
    end
})
NexusUI:CreateToggle(tabs["🍎 Frutas"], {
    Title = "🎲 Auto Roll Fruit", 
    Callback = function(v) 
        if v then startAutoRoll() else stopAutoRoll() end 
    end
})
NexusUI:CreateToggle(tabs["🍎 Frutas"], {
    Title = "🪄 Auto Spawn Fruit", 
    Callback = function(v) 
        if v then startAutoSpawn() else stopAutoSpawn() end 
    end
})
...
NexusUI:CreateToggle(tabs["💎 Farms"], {
    Title = "💎 Fragment Farm", 
    Callback = function(v) 
        if v then startFragmentFarm() else stopFragmentFarm() end 
    end
})
NexusUI:CreateToggle(tabs["💎 Farms"], {
    Title = "🦴 Bones Farm", 
    Callback = function(v) 
        if v then startBonesFarm() else stopBonesFarm() end 
    end
})
NexusUI:CreateToggle(tabs["💎 Farms"], {
    Title = "💰 Bounty Hunt", 
    Callback = function(v) 
        if v then startBountyHunt() else stopBountyHunt() end 
    end
})
...
NexusUI:CreateToggle(tabs["🏃 Move"], {
    Title = "🦘 Jumpspeed", 
    Callback = function(v) 
        if v then startJumpspeed() else stopJumpspeed() end 
    end
})
NexusUI:CreateToggle(tabs["🏃 Move"], {
    Title = "🚫 No Clip", 
...
NexusUI:CreateToggle(tabs["⚙️ Auto"], {
    Title = "🌀 Auto Haki", 
    Callback = function(v) 
        if v then startAutoHaki() else stopAutoHaki() end 
    end
})
NexusUI:CreateToggle(tabs["⚙️ Auto"], {
    Title = "⭐ Auto Skill", 
...
NexusUI:CreateToggle(tabs["⚙️ Auto"], {
    Title = "👑 Auto V4", 
    Callback = function(v) 
        if v then startAutoV4() else stopAutoV4() end 
    end
})
NexusUI:CreateToggle(tabs["⚙️ Auto"], {
    Title = "🧬 Auto Race", 
    Callback = function(v) 
        if v then startAutoRace() else stopAutoRace() end 
    end
})
NexusUI:CreateToggle(tabs["⚙️ Auto"], {
    Title = "📊 Auto Stats", 
    Callback = function(v) 
        if v then startAutoStats() else stopAutoStats() end 
    end
})
...
NexusUI:CreateToggle(tabs["🛍️ Loja"], {
    Title = "🍎 Buy Fruits", 
    Callback = function(v) 
        if v then startShopFruits() else stopShopFruits() end 
    end
})
NexusUI:CreateToggle(tabs["🛍️ Loja"], {
    Title = "👊 Buy Styles", 
    Callback = function(v) 
        if v then startShopStyles() else stopShopStyles() end 
    end
})
NexusUI:CreateToggle(tabs["🛍️ Loja"], {
    Title = "⚔️ Buy Swords", 
    Callback = function(v) 
        if v then startShopSword() else stopShopSword() end 
    end
})
NexusUI:CreateToggle(tabs["🛍️ Loja"], {
    Title = "🔫 Buy Guns", 
    Callback = function(v) 
        if v then startShopGuns() else stopShopGuns() end 
    end
})
NexusUI:CreateToggle(tabs["🛍️ Loja"], {
    Title = "💍 Buy Accessories", 
    Callback = function(v) 
        if v then startShopAcc() else stopShopAcc() end 
    end
})
NexusUI:CreateToggle(tabs["🛍️ Loja"], {
    Title = "📦 Buy Materials", 
    Callback = function(v) 
        if v then startShopMat() else stopShopMat() end 
    end
})
NexusUI:CreateToggle(tabs["🛍️ Loja"], {
    Title = "💎 Buy Fragments", 
    Callback = function(v) 
        if v then startShopFrag() else stopShopFrag() end 
    end
})
NexusUI:CreateToggle(tabs["🛍️ Loja"], {
    Title = "🦴 Buy Bones", 
    Callback = function(v) 
        if v then startShopBones() else stopShopBones() end 
    end
})
NexusUI:CreateToggle(tabs["🛍️ Loja"], {
    Title = "📊 Buy Stats", 
    Callback = function(v) 
        if v then startShopStats() else stopShopStats() end 
    end
})
...
NexusUI:CreateToggle(tabs["👀 ESP"], {
    Title = "👤 ESP Players", 
    Callback = function(v) 
        if v then startESP("players", Color3.fromRGB(255, 0, 0)) else stopESP("players") end 
    end
})
NexusUI:CreateToggle(tabs["👀 ESP"], {
    Title = "🍎 ESP Fruits", 
    Callback = function(v) 
        if v then startESP("fruits", Color3.fromRGB(255, 0, 255)) else stopESP("fruits") end 
    end
})
NexusUI:CreateToggle(tabs["👀 ESP"], {
    Title = "📦 ESP Chests", 
    Callback = function(v) 
        if v then startESP("chests", Color3.fromRGB(255, 215, 0)) else stopESP("chests") end 
    end
})
NexusUI:CreateToggle(tabs["👀 ESP"], {
    Title = "🎯 ESP Bosses", 
    Callback = function(v) 
        if v then startESP("bosses", Color3.fromRGB(255, 50, 50)) else stopESP("bosses") end 
    end
})
NexusUI:CreateToggle(tabs["👀 ESP"], {
    Title = "🌊 ESP Sea", 
    Callback = function(v) 
        if v then startESP("sea", Color3.fromRGB(0, 0, 255)) else stopESP("sea") end 
    end
})
...
NexusUI:CreateToggle(tabs["🎮 Extra"], {
    Title = "🎯 Aimlock", 
    Callback = function(v) 
        if v then startAimlock() else stopAimlock() end 
    end
})
...
-- NOTIFICAÇÃO FINAL
notify("NEXUS v7.0.9", "✅ Script carregado com sucesso!\n🔍 Diagnóstico de entidades ativo\n📦 Super Farm otimizado\n⚡ Todas as funções prontas", 5)
print("✅ NEXUS v7.0.9 - CARREGADO COM SUCESSO NO DELTA EXECUTOR")
print("🔍 Pressione F9 para ver o console de diagnóstico")
