    ...
    if mt then
        ...
        mt.__namecall = function(self, ...)
            ...
            if method == "FireServer" and type(args[1]) == "string" then
                local blocked = {"Kick","Ban","Teleport","Flag","Report","Detect","AntiCheat","Log","Alert","NotifyAdmin","SendLog","Spectate","Moderator","Admin","Check","Verify","Validate"}
...
task.spawn(function() while true do task.wait(randomDelay(15,35)) pcall(function() if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then VirtualUser:ClickButton1(Vector2.new(math.random(300,900),math.random(300,600))) end end) end end)
task.spawn(function() while true do task.wait(randomDelay(250,350)) pcall(function() if player.Character and player.Character:FindFirstChild("Humanoid") then player.Character.Humanoid:Move(Vector3.new(math.random(-1,1),0,math.random(-1,1))) end end) end end)
...
-- OTIMIZAÇÃO
-- ============================================================
pcall(function() settings().Rendering.QualityLevel = 1 Lighting.GlobalShadows = false Lighting.Brightness = 2 if Workspace.Terrain then Workspace.Terrain.WaterWaveSize = 0 end end)
...
local function findBoss(name)
    ...
    for _, o in pairs(Workspace:GetDescendants()) do
        if o.Name:find(name) and o:FindFirstChild("Humanoid") and o:FindFirstChild("HumanoidRootPart") and o.Humanoid.Health > 0 then
            return o
        end
    end
    return nil
...
local function collectItem(name)
    for _, o in pairs(Workspace:GetDescendants()) do
        pcall(function()
            if o.Name:lower():find(name:lower(), 1, true) and player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                o.CFrame = player.Character.HumanoidRootPart.CFrame
            end
        end)
    end
end
...
task.spawn(function()
    while true do
        ...
        -- GODMODE
        if godmodeEnabled or superFarmEnabled then
            pcall(function()
                local hum = player.Character:FindFirstChildOfClass("Humanoid")
        ...
        if killAuraEnabled then
            pcall(function()
                local hrp = player.Character.HumanoidRootPart
                for _, o in pairs(Workspace:GetDescendants()) do
                    if o:IsA("Model") and o ~= player.Character and o:FindFirstChild("Humanoid") and o:FindFirstChild("HumanoidRootPart") then
        ...
        if superFarmEnabled then
            pcall(function()
                ...
                if t % 3 < 0.1 then useSkill(Enum.KeyCode.Z) end
            end)
        end
        ...
        for _, sd in pairs(seaList) do
            if sd[1] and t % 10 < 0.1 then
                pcall(function()
                    local s = findSea(sd[2])
                    if s then
                        tp(s.HumanoidRootPart.Position)
                        for _ = 1, 5 do
                            attack()
                            task.wait(0.3)
                        end
        ...
        if beliFarmEnabled and t % 10 < 0.1 then
            pcall(function()
                local r = ReplicatedStorage:FindFirstChild("Remotes")
                if r and r:FindFirstChild("CommF_") then
                    r.CommF_:InvokeServer("AddMoney", 10000)
                end
            end)
        end
        ...
        if moveTpIslandEnabled and t % 30 < 0.1 then
            local il = SEA_DATA[currentSea].islands[math.random(1, #SEA_DATA[currentSea].islands)]
            tp(il[2])
        end

        -- AUTOMÁTICOS
        if autoSkillEnabled and t % 5 < 0.1 then
            pcall(function()
        ...
        -- LOJA
        if shopFruitsEnabled and t % 300 < 0.1 then
            pcall(function()
                buyItem("Kitsune")
                buyItem("Dragon")
                buyItem("Leopard")
        ...
        if shopStatsEnabled and t % 5 < 0.1 then
            pcall(function()
                local r = ReplicatedStorage:FindFirstChild("Remotes")
                if r and r:FindFirstChild("CommF_") then
...
-- Criar todas as abas
for _, tabName in pairs({"⚔️ Super Farm","🎯 Bosses","🌊 Sea","💎 Farms","🏃 Move","⚙️ Auto","🛍️ Loja","👀 Visual","🎮 Extra","🏝️ Ilhas"}) do
    tabs[tabName] = NexusUI:CreateTab(win, {Name = tabName, Icon = tabName:sub(1, 2)})
end
...
local bossList = {
    {"Dough King", function(v) farmBossDoughKing = v end},
    {"Soul Reaper", function(v) farmBossSoulReaper = v end},
    {"Cake Prince", function(v) farmBossCakePrince = v end},
    {"Rip Indra", function(v) farmBossRipIndra = v end},
    {"Darkbeard", function(v) farmBossDarkbeard = v end},
    {"Hydra", function(v) farmBossHydra = v end},
    {"Leviathan", function(v) farmBossLeviathan = v end},
}
for _, bd in pairs(bossList) do
    NexusUI:CreateToggle(tabs["🎯 Bosses"], {Title = bd[1], Callback = bd[2]})
end

-- SEA EVENTS
NexusUI:CreateSection(tabs["🌊 Sea"], "🌊 SEA EVENTS")
local seaList = {
    {"Marine Ship", function(v) farmSeaMarineShip = v end},
    {"Pirate Ship", function(v) farmSeaPirateShip = v end},
    {"Sea Beast", function(v) farmSeaBeast = v end},
}
for _, sd in pairs(seaList) do
    NexusUI:CreateToggle(tabs["🌊 Sea"], {Title = sd[1], Callback = sd[2]})
end

-- FARMS
NexusUI:CreateSection(tabs["💎 Farms"], "💎 FARMS")
local farmsList = {
    {"Fragmentos", function(v) fragmentFarmEnabled = v end},
    {"Ossos", function(v) bonesFarmEnabled = v end},
    {"Belis", function(v) beliFarmEnabled = v end},
    {"Scrolls", function(v) scrollFarmEnabled = v end},
}
for _, fd in pairs(farmsList) do
    NexusUI:CreateToggle(tabs["💎 Farms"], {Title = fd[1], Callback = fd[2]})
end

-- MOVIMENTO
NexusUI:CreateSection(tabs["🏃 Move"], "🏃 MOVIMENTAÇÃO")
local moveList = {
    {"Auto Flight", function(v) moveFlightEnabled = v end},
    {"Auto Swim", function(v) moveSwimEnabled = v end},
    {"TP Islands", function(v) moveTpIslandEnabled = v end},
}
for _, md in pairs(moveList) do
    NexusUI:CreateToggle(tabs["🏃 Move"], {Title = md[1], Callback = md[2]})
end

-- AUTOMÁTICOS
NexusUI:CreateSection(tabs["⚙️ Auto"], "⚙️ AUTOMAÇÕES")
local autoList = {
    {"Auto Skill", function(v) autoSkillEnabled = v end},
}
for _, ad in pairs(autoList) do
    NexusUI:CreateToggle(tabs["⚙️ Auto"], {Title = ad[1], Callback = ad[2]})
end
...
NexusUI:CreateToggle(tabs["👀 Visual"], {Title="ESP Players", Callback=function(v) espPlayersEnabled = v end})
...
NexusUI:CreateToggle(tabs["🎮 Extra"], {Title="Flight", Callback=function(v) flyEnabled = v end})
