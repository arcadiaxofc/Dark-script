-- ============================================================
-- NEXUS v7.0.9 - PARVUS STYLE - BLOX FRUITS COMPLETO
-- ============================================================
...
-- 5. SISTEMA DE TARGET (PRIORIDADE)
...
-- 7. SISTEMA DE FARM (CAIXA INVISÍVEL)
local Farm = {Enabled=false, Phase="collect", MobsKilled=0, MobsNeeded=10, LastQuestTime=0, BoxPart=nil, CollectedMobs={}, AutoQuest=true, AutoCollect=true, AutoBuy=true, GodMode=true}
function Farm.createBox()
    ...
    part.Name="NexusFarmBox" part.Size=Vector3.new(10,5,10) part.Anchored=true part.CanCollide=false part.Transparency=1
    Farm.BoxPart=part
end
function Farm.update()
    if not Farm.Enabled then if Farm.BoxPart then Farm.BoxPart:Destroy() Farm.BoxPart=nil end Farm.CollectedMobs={} return end
    if not player.Character or not player.Character:FindFirstChild("HumanoidRootPart") then return end
    ...
    if hum then hum:ChangeState(Enum.HumanoidStateType.Freefall) if Farm.GodMode then hum.Health=hum.MaxHealth end end
    ...
    if Farm.Phase=="collect" then
        ...
        for _,obj in pairs(Services.Workspace:GetDescendants()) do
            ...
            if obj:IsA("Model") and obj~=player.Character then
                local mHrp=obj:FindFirstChild("HumanoidRootPart") local mHum=obj:FindFirstChild("Humanoid")
                if mHrp and mHum and mHum.Health>0 and not Farm.CollectedMobs[obj] then
                    if not obj:FindFirstChild("Quest") and not obj:FindFirstChild("Talk") then
                        if(mHrp.Position-hrp.Position).Magnitude<=500 then
                            mHrp.CFrame=CFrame.new(Farm.BoxPart.Position+Vector3.new(math.random(-4,4),0,math.random(-4,4)))
                            Farm.CollectedMobs[obj]=true count=count+1
                        end
                    end
                end
            end
        ...
        for obj,_ in pairs(Farm.CollectedMobs) do
            if obj:FindFirstChild("Humanoid") and obj:FindFirstChild("HumanoidRootPart") then
                if obj.Humanoid.Health>0 then
                    ...
                    if(mobHrp.Position-Farm.BoxPart.Position).Magnitude>10 then mobHrp.CFrame=CFrame.new(Farm.BoxPart.Position+Vector3.new(math.random(-3,3),0,math.random(-3,3))) end
                    hrp.CFrame=CFrame.new(hrp.Position,mobHrp.Position)
                    Services.VirtualInputManager:SendMouseButtonEvent(0,0,0,true,game,0) task.wait(0.05)
                    Services.VirtualInputManager:SendMouseButtonEvent(0,0,0,false,game,0)
                    alive=alive+1
                else Farm.CollectedMobs[obj]=nil Farm.MobsKilled=Farm.MobsKilled+1 end
            else Farm.CollectedMobs[obj]=nil Farm.MobsKilled=Farm.MobsKilled+1 end
        end
        if alive<=3 then Farm.Phase="collect" end
        if Farm.MobsKilled>=Farm.MobsNeeded then Farm.Phase="quest" Farm.MobsKilled=0 Farm.CollectedMobs={} end
    end
    
    if Farm.Phase=="quest" and Farm.AutoQuest and tick()-Farm.LastQuestTime>60 then
        for _,obj in pairs(Services.Workspace:GetDescendants()) do
            if obj:IsA("Model") and obj:FindFirstChild("Humanoid") and obj:FindFirstChild("HumanoidRootPart") then
                if obj:FindFirstChild("Quest") or obj:FindFirstChild("Talk") or obj.Name:find("Quest") or obj.Name:find("Giver") then
                    ...
                    Farm.LastQuestTime=tick() Farm.MobsKilled=0 Farm.Phase="collect" break
                end
            end
        end
    end
    ...
    if Farm.AutoCollect and tick()%10<0.1 then
        for _,obj in pairs(Services.Workspace:GetDescendants()) do
            if obj.Name:find("Fruit") and obj:FindFirstChild("Handle") then
                hrp.CFrame=CFrame.new(obj.Handle.Position+Vector3.new(0,25,0)) task.wait(0.1) hrp.CFrame=CFrame.new(obj.Handle.Position) break
            end
        end
    end
    ...
    if Farm.AutoBuy and tick()%300<0.1 then
        local r=Services.ReplicatedStorage:FindFirstChild("Remotes")
        if r and r:FindFirstChild("CommF_") then r.CommF_:InvokeServer("BuyItem","Kitsune") r.CommF_:InvokeServer("BuyItem","Dragon") end
    end
end

-- 8. SISTEMA DE MODIFICAÇÕES
local Mods = {
    WalkSpeed={Enabled=false,Value=100}, JumpPower={Enabled=false,Value=150}, Fly={Enabled=false},
    NoClip={Enabled=false}, GodMode={Enabled=false}, KillAura={Enabled=false,Range=300}, Aimlock={Enabled=false,Range=300},
}
function Mods.update()
    if not player.Character then return end
    local hum=player.Character:FindFirstChildOfClass("Humanoid")
    local hrp=player.Character:FindFirstChild("HumanoidRootPart")
    if Mods.WalkSpeed.Enabled and hum then hum.WalkSpeed=Mods.WalkSpeed.Value elseif hum and hum.WalkSpeed~=16 then hum.WalkSpeed=16 end
    if Mods.JumpPower.Enabled and hum then hum.JumpPower=Mods.JumpPower.Value elseif hum and hum.JumpPower~=50 then hum.JumpPower=50 end
    if Mods.Fly.Enabled and hum then hum:ChangeState(Enum.HumanoidStateType.Freefall) if hrp then hrp.Velocity=Vector3.new(hrp.Velocity.X,10,hrp.Velocity.Z) end end
    if Mods.NoClip.Enabled then for _,p in pairs(player.Character:GetDescendants()) do if p:IsA("BasePart") then p.CanCollide=false end end end
    ...
    if Mods.Aimlock.Enabled and hrp then
        ...
        if target and target:FindFirstChild("HumanoidRootPart") then hrp.CFrame=CFrame.new(hrp.Position,target.HumanoidRootPart.Position) end
    end
end

-- 9. LOOP PRINCIPAL
task.spawn(function() while true do pcall(function() Farm.update() Mods.update() ESP.update() end) task.wait(0.1) end end)

-- 10. UI (ESTILO PARVUS)
local NexusUI = loadstring(game:HttpGet("https://raw.githubusercontent.com/arcadiaxofc/Dark-script/refs/heads/main/ui.lua"))()
local win = NexusUI:CreateWindow({Title="NEXUS v7.0.9", Subtitle="Parvus Style | Blox Fruits", Width=580, Height=500})
local tabs={}
for _,t in pairs({{"⚔️ Combat"},{"🚀 Farm"},{"👀 Visuals"},{"⚙️ Mods"},{"🏝️ Teleports"},{"🛑 Config"}}) do tabs[t[1]]=NexusUI:CreateTab(win,{Name=t[1]}) end
...
NexusUI:CreateSection(tabs["🚀 Farm"],"Auto Farm")
...
for _,island in pairs({{"🏴‍☠️ Pirate Starter",Vector3.new(1289,11,4191)},{"🌴 Jungle",Vector3.new(-1250,15,3850)},{"🏜️ Desert",Vector3.new(966,10,1100)},{"❄️ Frozen Village",Vector3.new(1150,25,4350)},{"🔒 Prison",Vector3.new(-5400,15,-1700)},{"☁️ Skylands",Vector3.new(-4850,750,1950)},{"🌹 Kingdom of Rose",Vector3.new(-1400,10,-1400)},{"🏯 Ice Castle",Vector3.new(7200,100,3500)},{"🐉 Hydra Island",Vector3.new(6200,80,2500)},{"🏰 Castle on the Sea",Vector3.new(4500,50,1200)}}) do
    NexusUI:CreateButton(tabs["🏝️ Teleports"],{Title=island[1],Callback=function() if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then local hrp=player.Character.HumanoidRootPart hrp.CFrame=CFrame.new(island[2]+Vector3.new(0,25,0)) task.wait(0.1) hrp.CFrame=CFrame.new(island[2]) end end})
end

NexusUI:CreateSection(tabs["🛑 Config"],"Settings")
NexusUI:CreateButton(tabs["🛑 Config"],{Title="🛑 DISABLE ALL",Callback=function() for k,v in pairs(Mods) do if type(v)=="table" and v.Enabled~=nil then v.Enabled=false end end Farm.Enabled=false ESP.Enabled=false notify("NEXUS","Tudo desligado!",3) end})
NexusUI:CreateButton(tabs["🛑 Config"],{Title="🚀 ENABLE ALL",Callback=function() Farm.Enabled=true Farm.Phase="collect" ESP.Enabled=true Mods.GodMode.Enabled=true Mods.KillAura.Enabled=true notify("NEXUS","Tudo ligado!",3) end})

print("NEXUS v7.0.9 - Parvus Style - Blox Fruits - Loaded")
