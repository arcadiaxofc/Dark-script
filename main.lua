-- ============================================================
-- NEXUS SUPREMO - MAIN (O CHEFE)
-- Este arquivo carrega a UI e o CORACAO
-- Cole APENAS este codigo no Delta
-- ============================================================
...
-- Tenta carregar a UI do GitHub
pcall(function()
    local uiCode = game:HttpGet("https://raw.githubusercontent.com/arcadiaxofc/Dark-script/refs/heads/main/ui.lua")
    if uiCode then
        local uiFunc = loadstring(uiCode)
        if uiFunc then
            local DiscordLib = uiFunc()
            if DiscordLib then
                UI_Carregada = true
                ...
                stopCh:Button("PARAR TUDO", function()
                    ...
                    _G.AutoKill = false; _G.SmartNight = false
                    _G.ESP_Enabled = false
                    DiscordLib:Notification("NEXUS", "Tudo parado!", "OK")
                end)
                
                DiscordLib:Notification("NEXUS 99 NOITES", "Script carregado!\nAtive as funcoes nos canais!", "OK")
            end
        end
    end
end)
...
-- ============================================================
-- PASSO 2: CARREGAR O CORACAO (SISTEMAS)
-- ============================================================
-- Cria uma pasta escondida no jogo
local folder = Instance.new("Folder")
folder.Name = "NexusCore"
folder.Parent = game:GetService("Workspace")

-- Cria o script com todos os sistemas
local coreScript = Instance.new("LocalScript")
coreScript.Name = "CoreHandler"
coreScript.Source = [===[
    -- ============================================================
    -- NEXUS 99 NOITES - CORACAO (SISTEMAS)
    -- ============================================================
    ...
    if _G.AutoWood == nil then _G.AutoWood = false end
    if _G.AutoFood == nil then _G.AutoFood = false end
    if _G.AutoGems == nil then _G.AutoGems = false end
    if _G.AutoFire == nil then _G.AutoFire = false end
    if _G.AutoCollect == nil then _G.AutoCollect = false end
    if _G.AutoDefense == nil then _G.AutoDefense = false end
    if _G.AutoKill == nil then _G.AutoKill = false end
    if _G.SmartNight == nil then _G.SmartNight = false end
    if _G.Range == nil then _G.Range = 50 end
    if _G.ESP_Enabled == nil then _G.ESP_Enabled = false end
    if _G.ESP_Players == nil then _G.ESP_Players = false end
    if _G.ESP_Items == nil then _G.ESP_Items = false end
    if _G.ESP_Enemies == nil then _G.ESP_Enemies = false end
    if _G.ESP_Range == nil then _G.ESP_Range = 200 end
            ...
            pcall(function()
                ...
                if _G.ESP_Enemies then
                    for _, obj in pairs(Workspace:GetDescendants()) do
                        ...
                        local bg = Instance.new("BillboardGui"); bg.Adornee = head; bg.Size = UDim2.new(0, 80, 0, 16); bg.AlwaysOnTop = true
                        bg.MaxDistance = _G.ESP_Range; bg.StudsOffset = Vector3.new(0, 3, 0); bg.Parent = CoreGui
                        local label = Instance.new("TextLabel", bg); label.Size = UDim2.new(1, 0, 1, 0); label.BackgroundTransparency = 0.5
    ...
    task.spawn(function()
        while task.wait(1) do
            if not _G.SmartNight then continue end
...
print("[NEXUS] Coracao injetado com sucesso!")
print("[NEXUS] Pasta: Workspace.NexusCore")
print("[NEXUS] Script: CoreHandler")
